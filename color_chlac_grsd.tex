...
This approach is similar to Color-CHLAC, but in order to efficiently merge the two features,
the original GRSD has to be simplified. As presented in \cite{GRSD10Humanoids} we were using
raytracing in order to find transitions between surface types of voxels. To merge the features,
raytracing was abandoned for simple checks of neighboring voxels, and the feature is no longer normalized.

These modifications allow us to create histograms which, like Color-CHLAC, are roughly additive.
If we break up objects into reasonable sizes, its histogram can be approximated by the sum of the
histograms of its partitioning. Unlike in Color-CHLAC, free space is considered, but we are looking
...
b=\frac{s(s+1)}{2}-s
\end{equation}
Previously only predifined surface types with an intuitive description were used (cylinder, sphere, plane, rim and corner),
based on an estimation of the radiuses of the 2 principal curves defining the surface, $r_{min}$ and $r_{max}$ \cite{Marton10IROS}.
Since these values have physical meaning, we can categorize surfaces into predifined types, but 
\todo{now we are dividing => more surface types, chosen to roughly match Color-CHLAC to have the same "weighting"}
...
Since we do not use raycasting but consider only the direct neighbors
