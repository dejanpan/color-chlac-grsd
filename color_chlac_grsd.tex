...
(Color-CHLAC~\cite{kanezaki2010tvc}). The powerfullness of the descriptor is thus in that
...
both in 2D as well in 3D data domain. From various versions of Point Feature Histograms~\cite{FPFH},
Viewpoint Feature Histogram~\cite{vfh}, NARF~\cite{steder10irosws}, SIFT~\cite{lowe04distinctive} 
and SURF~\cite{surf} are just to name a few. Yet it seems that there is none that can capture 
all important and obvious properties (such as geometry and color are) and can be out-of-the 
box applied on the data coming out of the new sensing technologies such Kinect~\cite{kinect}. 
